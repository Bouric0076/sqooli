...
export async function GET() {
  try {
    const backendUrl = `${process.env.BACKEND_API_URL}/Curricula`;
...
export async function POST(req: Request) {
  try {
    ...
    const response = await axiosClient.post(
      `${process.env.BACKEND_API_URL}/Curricula`,
      body
    );
    ...
    console.error("Curricula creation failedd:", error?.response?.data || error.message);

    return NextResponse.json(
      { message: "Failed to create Curricula" },
      { status: 500 }
    );
  }
}
...
/* ================= PUT ================= */
// Update Curricula (optional, example)
export async function PUT(req: Request) {
  try {
    ...
    if (!body.id) {
      return NextResponse.json(
        { message: "Curricula id is required for update" },
        { status: 400 }
      );
    }
   ...
   const response = await axiosClient.put(
    ...
    return NextResponse.json(
      { message: "Failed to update Curricula "+error.message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!body.id) {
      return NextResponse.json(
        { message: "Curricula id is required for delete" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Failed to delete Curricula "+error.message },
      { status: 500 }
    );
  }
}