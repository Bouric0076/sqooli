...
export default function LessonCardTeacher({
  lesson,
  viewMode,
}: LessonCardProps) {
  ...
  const handleViewLesson = async (lesson: Lesson) => {
    ...
    await setActiveLesson({ id: lesson.id, title: lesson.title });

    router.push(`/teacher/curriculum/${lesson?.curriculum}/lessons/create-lesson`);
  };
