...
import { IntroModal } from '../assignments/components/IntroModal';
import { QuizModal } from '../assignments/components/QuizModal';
import { SubmittedList } from '../assignments/components/SubmittedList';
...
function Page(props: Record<string, unknown>) {
  ...
  // 2. Open the quiz modal for the active exams
  ...
  // 3. Fetch lessons and extract individual exams
  useEffect(() => {
    setLoading(true);
    getBookingLessons()
      .then((data) => {
          ...
          booking.lesson?.sections?.forEach((section: any) => {
            section.lectures?.forEach((lecture: any) => {
              lecture.items?.forEach((item: any) => {
                if (item.type === "Exam" && item.data?.sections) {
                  ...
                  extractedAssignments.push({
                    ...
                    assignmentId: item.referenceId,
                    title: item.title || booking.lesson?.name || 'Exam',
    ...
    <div style={styles.page}>
      ...
      <div style={styles.tabsWrapper}>
        {(['upcoming', 'submitted'] as const).map(tab => (
          ...
          >
            {tab === 'upcoming' ? 'Upcoming Exam' : 'Submitted Exams'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'submitted' && (
   <SubmittedList type="exam" />
      )}

      {modal.mode === 'intro' && (
        <IntroModal assignment={modal.assignment} onClose={closeModal}  type="exam" onStartQuiz={openQuiz} />
      )}

      {modal.mode === 'quiz' && (
        <QuizModal questions={questions} assignment={modal.assignment} type="exam" onClose={closeModal} />
      )}
    </div>
  );
}
