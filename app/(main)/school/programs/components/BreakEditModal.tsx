...
import { inputBase, HH, MM, AP, parseTime, fmtTime } from "../constants";
import type { BreakEntry, TimeValue } from "../constants";
...
export const BreakEditModal: FC<BreakEditModalProps> = ({
  ...
  const handleSave = () => {
    onSave({ name: name.trim() || "Break", from: fmtTime(from), to: fmtTime(to) });
    onClose();
  };
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
          <button
            onClick={onClose}
            style={{
              border: "1.5px solid #d1d5db", borderRadius: 24, padding: "8px 20px",
