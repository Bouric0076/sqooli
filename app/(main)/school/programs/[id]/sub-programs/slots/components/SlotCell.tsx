...
export function SlotCell({
  ...
  // ✅ ACCEPTED
  if (slot.type === "accepted") {

    const subjectColor = slot.subject
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {slot.avatarInitials && (
            <div
              ...
              style={{
                background: avatarColor(slot.avatarInitials)
              }}
            >
              {slot.avatarInitials}
            </div>
          ...
          <span className="text-[13px] font-bold text-[#027A48] truncate">
            {slot.code}
          </span>

          {subjectColor && (
            <span
              className="text-[9px] font-bold px-1.5 py-0.5 rounded-full ml-auto flex-shrink-0 border"
              style={{
        ...
        <span className="text-[11px] text-[#027A48] font-medium opacity-80 truncate">
          {slot.teacherName}
        </span>
      </button>
    );
  ...
  if (slot.type === "pending") {
    ...
    const pendingN = invites.filter(
      i => i.status === "pending"
    ).length;

    const declinedN = invites.filter(
      i => i.status === "declined"
    ).length;
        ...
        <div className="flex items-center gap-1">
          ...
          {isSelected
            ? (
              <SquareCheck
                size={14}
                className="text-[#3B9EFF] mr-0.5 flex-shrink-0"
              />
            )
            : (
              <Square
                size={14}
                className="text-[#CBD5E1] mr-0.5 flex-shrink-0"
              />
            )}
          ...
          {invites.length > 3 && (
            <span className="text-[10px] font-bold text-[#B45309] ml-0.5">
              +{invites.length - 3}
            </span>
          )}
        </div>
  ...
  if (slot.type === "break") {
    return (
      <div
        className="w-full h-[72px] border rounded-2xl flex flex-col items-center justify-center gap-1
  ...
  if (slot.type === "holiday") {
    return (
      ...
      >
        ...
        <span className="text-[12px] font-bold text-center px-2">
