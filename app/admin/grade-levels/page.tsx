...
import { getCurriculums, getEducationLevels, getGradeLevels } from "@/app/helpers/lookups";
...
import { addGradeLevel, DeleteGradeLevel, UpdateGradeLevel } from "@/app/lib/grade-levels";
...
const SectionTitle = "Grade Levels";
export default function Page() {
  ...
  const [editingInfo, setEditingInfo] = useState<GradeLevel | null>(null);
  const [infoToDelete, setinfoToDelete] = useState<GradeLevel | null>(null);

  const form = useForm<GradeLevel>({
    defaultValues: {
      name: "",
      id: "",
      curriculumId: "",
...
const fetchData = async () => {
  ...
  try {
    const [ curriculums,gradeLevels] = await Promise.all([
...
// },[watch(s)]);
  ...
  const openEditModal = (row: GradeLevel) => {
    reset();
    setValue("id",row.id);
  ...
  const openDeleteModal = (row: GradeLevel) => {
    setinfoToDelete(row);
    ...
    try {
      if (editingInfo) {
        ...
        // Optimistic UI update
           ...
           await UpdateGradeLevel(payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Optimistic UI update
await DeleteGradeLevel(payload);
      closeDeleteModal();
    } catch (error) {
  ...
  const filtered = gradeLevels.filter((p) =>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: "#FAFAFA" }}>
                {[" Name", "Curricula","EducationLevel","Action"].map((h) => (
...
<div className="flex flex-col  mb-5 gap-5">
        ...
        >
          ...
          />
                   ...
                   <FormField className="text-sm font-medium text-gray-700" label="GradeLevel Name" error={errors.name?.message}>
             <TextInput
               type="text"
               placeholder="Enter GradeLevel Name"
               {...register("name", { required: "GradeLevel Name is required" })}
             />
           </FormField>
