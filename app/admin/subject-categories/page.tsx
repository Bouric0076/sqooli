...
import { getCurriculums, getSubjectCategories } from "@/app/helpers/lookups";
...
import { addSubjectCategory, DeleteSubjectCategory, UpdateSubjectCategory } from "@/app/lib/subject-category";
...
export default function Page() {
  ...
  const [editingCurriculum, setEditingCurriculum] = useState<SubjectCategory | null>(null);
  const [curriculumToDelete, setCurriculumToDelete] = useState<SubjectCategory | null>(null);

  const form = useForm<SubjectCategory>({
    defaultValues: {
      name: "",
      id: "",
    },
  ...
  const openEditModal = (info: SubjectCategory) => {
    reset();
    setValue("id",info.id);
     setValue("name",info.name);
    setEditingCurriculum(info);
    setIsAddEditModalOpen(true);
  };
  ...
  const openDeleteModal = (curriculum: SubjectCategory) => {
    ...
    try {
      if (editingCurriculum) {
        ...
        // Optimistic UI update
           ...
           await UpdateSubjectCategory(payload);
       ...
       await addSubjectCategory(payload)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Optimistic UI update
await DeleteSubjectCategory(payload);
      closeDeleteModal();
    } catch (error) {
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ margin: "3px 0 0 0", fontSize: "13px", color: "#94A3B8", fontWeight: 400 }}>
            Manage and monitor academic Subject Categories
          </p>
        </div>
...
<div className="flex flex-col  mb-5 gap-5">
           ...
           <FormField className="text-sm font-medium text-gray-700" label="Subject Category Name" error={errors.name?.message}>
             <TextInput
               type="text"
               placeholder="Enter Program Name"
               {...register("name", { required: "Subject Category Name is required" })}
             />
           </FormField>
