...
import { getCurriculums, getEducationLevels } from "@/app/helpers/lookups";
...
import { addEducationLevel, DeleteEducationLevel, UpdateEducationLevel } from "@/app/lib/education-levels";
...
const SectionTitle = "Education Levels";
export default function Page() {
  ...
  const [editingInfo, setEditingInfo] = useState<EducationLevel | null>(null);
  const [infoToDelete, setinfoToDelete] = useState<EducationLevel | null>(null);

  const form = useForm<EducationLevel>({
    defaultValues: {
      name: "",
      id: "",
      curriculumId: "",
...
const fetchData = async () => {
  ...
  try {
    const [educationLevels, curriculums] = await Promise.all([
      getEducationLevels({}),
      getCurriculums({})
    ]);
  ...
  const openEditModal = (row: EducationLevel) => {
    reset();
    setValue("id",row.id);
  ...
  const openDeleteModal = (row: EducationLevel) => {
    setinfoToDelete(row);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Optimistic UI update
await DeleteEducationLevel(payload);
      closeDeleteModal();
    } catch (error) {
  ...
  const filtered = educationLevels.filter((p) =>
...
<div className="flex flex-col  mb-5 gap-5">
        ...
        >
          ...
          />
                   ...
                   <FormField className="text-sm font-medium text-gray-700" label="EducationLevel Name" error={errors.name?.message}>
             <TextInput
               type="text"
               placeholder="Enter EducationLevel Name"
               {...register("name", { required: "EducationLevel Name is required" })}
             />
           </FormField>
