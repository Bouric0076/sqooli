...
import { getCurriculums, getEducationLevels, getGradeLevels, getSubjectCategories, getSubjects, getTopics } from "@/app/helpers/lookups";
...
import { UpdateTopic, addTopic, DeleteTopic } from "@/app/lib/topics";
...
export default function Page() {
 ...
 const [subjects, setSubjects] = useState<Subject[]>([]);
  ...
  const [editingInfo, setEditingInfo] = useState<Topic | null>(null);
  const [infoToDelete, setinfoToDelete] = useState<Topic | null>(null);
...
// const subjectCategoryId = watch("subjectCategoryId");
...
const fetchData = async () => {
  ...
  try {
    const [curriculums,topics] = await Promise.all([
      getCurriculums({}),
         getTopics({})
    ]);
...
useEffect(() => {
  ...
  const openEditModal = (row: Topic) => {
    ...
    setValue("id",row.id);
     ...
     setValue("subjectId", row.subjectId?.toString());
    setEditingInfo(row);
    setIsAddEditModalOpen(true);
  };
  ...
  const openDeleteModal = (row: Topic) => {
    setinfoToDelete(row);
...
await DeleteTopic(payload);
  ...
  const filtered = topics.filter((p) =>
  ...
  <div className="grid grid-cols-2 gap-6">
        ...
        >
          ...
          />
            ...
            <FormField className="text-sm font-medium text-gray-700" label="Topic Name" error={errors.name?.message}>
             <TextInput
               type="text"
