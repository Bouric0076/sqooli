...
import { getCurriculums, getEducationLevels, getGradeLevels, getSubjectCategories, getSubjects } from "@/app/helpers/lookups";
...
import { Curriculum } from "../curriculums/page";
import { SelectInput } from "@/app/components/ui/form/SelectInput";
import { EducationLevel } from "../education-levels/page";
import { GradeLevel } from "../grade-levels/page";
import { SubjectCategory } from "../subject-categories/page";
import { UpdateSubject, addSubject, DeleteSubject } from "@/app/lib/subject";
...
export default function Page() {
 ...
 const [subjects, setSubjects] = useState<Subject[]>([]);
  ...
  const [editingInfo, setEditingInfo] = useState<Subject | null>(null);
  const [infoToDelete, setinfoToDelete] = useState<Subject | null>(null);
...
const fetchData = async () => {
  ...
  try {
    const [curriculums,subjectCategories,subjects] = await Promise.all([
      getCurriculums({}),
...
setcurriculums(curriculums || []);
    setSubjectCategories(subjectCategories || []);
...
useEffect(() => {
  ...
  const loadEducationLevels = async () => {
    try {
      ...
      // clear children
      // form.setValue("educationLevelId", "");
      // form.setValue("gradeLevelId", "");
...
// },[watch(s)]);
  ...
  const openEditModal = (row: Subject) => {
    ...
    setValue("id",row.id);
     ...
     setValue("colorCode",row.colorCode);
     setValue("curriculumId", row.curriculumId?.toString());
     setValue("educationLevelId", row.educationLevelId?.toString());
     setValue("gradeLevelId", row.gradeLevelId?.toString());
     setValue("categoryId", row.categoryId?.toString());
    setEditingInfo(row);
    setIsAddEditModalOpen(true);
  };
  ...
  const openDeleteModal = (row: Subject) => {
    setinfoToDelete(row);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
          ...
          educationLevelId: form.getValues("educationLevelId"),
          gradeLevelId: form.getValues("gradeLevelId"),
...
await DeleteSubject(payload);
  ...
  const filtered = subjects.filter((p) =>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ margin: "3px 0 0 0", fontSize: "13px", color: "#94A3B8", fontWeight: 400 }}>
            Manage and monitor academic  {SectionTitle}
          </p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: "#FAFAFA" }}>
                {[" Name", "Curricula","EducationLevel","Grade Level","Action"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} style={{ padding: "40px", textAlign: "center", color: "#94A3B8" }}>Loading {SectionTitle}...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: "40px", textAlign: "center", color: "#94A3B8" }}>No {SectionTitle} found.</td>
                </tr>
              ) : (
  ...
  <div className="grid grid-cols-2 gap-6">
        ...
        >
          <Controller
            name="curriculumId"
            control={control}
            rules={{ required: "Select your curriculum" }}
            render={({ field }) => (
              <SelectInput
                placeholder="Select..."
                value={field.value}
                onChange={field.onChange}
                options={curriculums.map((g) => ({
                  label: g.name,
                  value: g.id.toString(),
                }))}
              />
        ...
        >
          <Controller
            name="gradeLevelId"
            control={control}
            rules={{ required: "Select your grade level" }}
            render={({ field }) => (
              <SelectInput
                placeholder="Select..."
                value={field.value}
        ...
        >
          <Controller
            name="categoryId"
            control={control}
            rules={{ required: "Select subject category" }}
            render={({ field }) => (
              <SelectInput
                placeholder="Select..."
                value={field.value}
                onChange={field.onChange}
                options={subjectCategories.map((g) => ({
                  label: g.name,
                  value: g.id.toString(),
                }))}
              />
          ...
          />
                   ...
                   <FormField className="text-sm font-medium text-gray-700" label="Subject Name" error={errors.name?.message}>
             <TextInput
               type="text"
        ...
        <FormField className="text-sm font-medium text-gray-700" label="Subject colorCode" error={errors.colorCode?.message}>
             <TextInput
               type="text"
               placeholder="Enter colorCode e.g #fff"
               {...register("colorCode", { required: "Subject colorCode is required" })}
             />
           </FormField>
