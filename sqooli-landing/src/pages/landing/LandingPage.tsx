...
import worshipBackground from '../../assets/images/hero/IMG_6358 2.webp'
import speakerPortrait from '../../assets/images/hero/thumbnail 2.webp'
import sqooliMark from '../../assets/images/hero/Group.png'
import studentBackground from '../../assets/images/hero/student-background.webp'
import childrenCollage from '../../assets/images/how-it-works/children-collage.webp'
import contactKids from '../../assets/images/contact/contact-kids.webp'
import lucyAvatar from '../../assets/images/whats-popular/teacher.webp'
import classProgramArtwork from '../../assets/images/whats-popular/classes.webp'
import upcomingProgramArtwork from '../../assets/images/programs/teachers-change-world.webp'
...
const UDBC_URL = 'https://udbc.sqooli.africa'
const HERO_TRANSITION_MS = 720

const UDBC_POPULAR_CARDS = {
  Programs: [
    { title: 'Certificate Pathway', tags: ['Lisha Kondoo Zangu', 'Part Time'], image: udbcCommunityTeam, imageAlt: 'UDBC student community', detail: 'Grades 1–3 · Approx. 6 months', action: 'View Certificate', href: UDBC_URL },
    { title: 'Diploma Pathway', tags: ['Lisha Kondoo Zangu', 'Full Time'], image: udbcDigitalLearningCommunity, imageAlt: 'Students learning together with digital devices', detail: 'Grades 1–7 · Approx. 14 months', action: 'View Diploma', href: UDBC_URL },
  ],
  Enrolments: [
    { title: 'July 2026 Intake', tags: ['Grade 1 Open', 'New Students'], image: udbcEnrolmentSupport, imageAlt: 'UDBC team supporting enrolments', detail: 'Begin as a Potential Shepherd', action: 'Enrol Now', href: `${UDBC_URL}/intake-landing` },
  ],
} as const
...
export default function LandingPage() {
  ...
  const udbcCards = tab === 'Programs' || tab === 'Enrolments' ? UDBC_POPULAR_CARDS[tab] : null
  const visiblePopularCards = udbcCards ?? items
  ...
  const ongoingPrograms = [
    { level: 'Part Time', pathway: 'Certificate · Level 1', title: 'Lisha Kondoo Zangu', duration: 'Grades 1–3 · Approx. 6 months', image: udbcCommunityTeam, imageAlt: 'UDBC students standing together at a community event', copy: 'Build strong theological foundations through a guided, grade-by-grade learning path.', benefits: ['Admission fee (one-time): TZS 30,000', 'Grade fees: G1 35K · G2 40K · G3 45K', 'Exam and graduation fee: TZS 20,000 / grade', 'Award: UDBC Certificate of Theological Studies — Title: Shepherd (Mchungaji)'] },
    { level: 'Full Time', pathway: 'Diploma · Level 2', title: 'Lisha Kondoo Zangu', duration: 'Grades 1–7 · Approx. 14 months', image: udbcDigitalLearningCommunity, imageAlt: 'Students learning together with digital devices', copy: 'Deepen your studies through the complete diploma pathway, from first grade to graduation.', benefits: ['Admission fee (one-time): TZS 30,000', 'Grade fees: G1–G3 as Certificate + G4 50K · G5 55K · G6 60K · G7 70K', 'Exam and graduation fee: TZS 20,000 / grade', 'Award: UDBC Diploma in Theological Studies — Title: Senior Resident Pastor'] },
  ]
  const listings = [{ name: 'Ufufuo Digital Bible College', students: 'Digital college', location: 'East Africa', website: 'https://udbc.sqooli.africa' }]
  ...
  useEffect(() => {
    // Decode the alternate hero before it becomes visible so the crossfade
    // never exposes an unpainted image layer.
    [studentBackground, worshipBackground].forEach((image) => {
  ...
  useEffect(() => { if (previousSlide === null) return undefined; const timer = window.setTimeout(() => { setPreviousSlide(null); setIsHeroTransitioning(false) }, HERO_TRANSITION_MS); return () => window.clearTimeout(timer) }, [previousSlide])
  useEffect(() => {
  ...
  const goToTutorProfile = () => window.location.assign('/tutors/profile')
  return <main id="top">
    <Header />
    ...
    <section className={`hero hero-figma slide-${activeSlide} ${slide.student ? 'student-slide' : ''} ${previousSlide !== null ? 'hero-has-previous' : ''} ${isHeroTransitioning ? 'hero-transitioning' : ''} ${reducedMotion ? 'reduce-motion' : ''}`}>
      {previousSlide !== null && <><img className="hero-background hero-outgoing" src={slides[previousSlide].image} alt="" decoding="async" /><div className="hero-outgoing-copy" aria-hidden="true"><h1>{slides[previousSlide].title}</h1><p>{slides[previousSlide].description}</p></div></>}{<img className="hero-background hero-incoming" src={slide.image} alt="" fetchPriority="high" decoding="async" />}
      ...
      <div className="container hero-copy">
        ...
        <div className="hero-actions"><a className="button enrol" href={slide.student ? '/search' : UDBC_URL}>{slide.student && <Search size={18} />} {slide.action}</a>{!slide.student && <a className="button learn" href="#programs">Learn More</a>}</div>
      </div>
      {slide.portrait && <><img className="speaker-portrait" src={speakerPortrait} alt="Ufufuo Digital Bible College speaker" decoding="async" /><span className="powered">Powered by <img src={sqooliMark} alt="Sqooli" /><small>A class of your own</small></span></>}<svg className="hero-wave" viewBox="0 0 1440 92" preserveAspectRatio="none" aria-hidden="true"><path d="M0 42 C125 90 235 15 382 48 C510 78 604 88 725 60 C850 31 930 4 1050 29 C1195 60 1300 89 1440 55 L1440 92 L0 92 Z" /></svg>
    </section>

    <section id="partners"><PartnerLogos compact /></section>

    <section id="popular" className="popular native-popular"><div className="container"><div className="section-heading"><h2>Discover What’s Popular</h2><a className="text-link" href="/popular">Explore More</a></div><div className="popular-toolbar"><div className="tabs" role="tablist" aria-label="Popular categories">{popularTabs.map(item => <button key={item} role="tab" aria-selected={tab === item} className={tab === item ? 'active' : ''} onClick={() => { setTab(item); setActiveCard(0) }}>{item}</button>)}</div>{visiblePopularCards.length > 1 && <div className="popular-arrows"><button type="button" aria-label="Previous cards" onClick={() => setActiveCard((activeCard + visiblePopularCards.length - 1) % visiblePopularCards.length)}><ArrowLeft /></button><button type="button" aria-label="Next cards" onClick={() => setActiveCard((activeCard + 1) % visiblePopularCards.length)}><ArrowRight /></button></div>}</div><div className={`native-card-rail ${udbcCards ? `native-card-rail--${udbcCards.length}` : ''}`}>{visiblePopularCards.map((item, index) => { const udbcCard = udbcCards?.[index]; const itemKey = typeof item === 'string' ? item : item.title; return <article className={`native-card ${udbcCard ? 'udbc-popular-card' : ''} ${index === activeCard ? 'featured' : ''}`} key={udbcCard?.title ?? itemKey}><div className="card-top"><h3>{udbcCard?.title ?? (tab === 'Tutors' ? 'Jane Doe' : tab === 'Questions' ? 'What is an atom?' : 'The Ultimate Math Camp Kenya April 2026')}</h3><button type="button" aria-label="Card options" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setOpenCardMenu(openCardMenu === index ? null : index)}><MoreVertical size={20} /></button>{openCardMenu === index && <div className="card-menu" role="menu"><button type="button" role="menuitem" onClick={() => { setNotice('Saved to your list'); setOpenCardMenu(null) }}><Bookmark />Save</button><button type="button" role="menuitem" onClick={() => { setNotice('Share link copied'); setOpenCardMenu(null) }}><Share2 />Share</button></div>}</div><div className="tag-row">{udbcCard ? udbcCard.tags.map(tag => <span key={tag}>{tag}</span>) : <><span>{tab === 'Questions' ? 'Chemistry' : 'Math'}</span>{tab === 'Tutors' && <><span>Eng</span><span>Physics</span></>}</>}</div><div className={`card-art art-${index}`}>{udbcCard ? <img src={udbcCard.image} alt={udbcCard.imageAlt} loading="lazy" decoding="async" /> : tab === 'Classes' ? <img src={classProgramArtwork} alt="Teachers Change the world classroom programme" loading="lazy" decoding="async" /> : <span>{tab === 'Questions' ? 'Lucy asks a question' : 'Learning together'}</span>}</div><p className={udbcCard ? 'udbc-card-detail' : 'card-rating'}>{udbcCard ? udbcCard.detail : '4.5 ★★★★★'}</p><div className="card-bottom"><span>{udbcCard ? 'Ufufuo Digital Bible College' : tab === 'Questions' ? '20 votes · 20 helpful' : tab === 'Tutors' ? '200 lessons taught' : 'KES 200.00'}</span>{udbcCard ? <a href={udbcCard.href}>{udbcCard.action}</a> : <button type="button" onClick={() => { if (tab === 'Tutors') goToTutorProfile(); else if (tab === 'Questions') window.location.href = '/questions'; else window.location.href = '/courses/detail' }}>{tab === 'Tutors' ? 'View Tutor' : tab === 'Questions' ? 'View Question' : 'View Class'}</button>}</div></article>})}</div>{notice && <p className="action-notice" role="status">{notice}</p>}</div></section>
        ...
        {programView === 'curriculum' ? (
          <div role="region" aria-label="Ongoing programs" className="upcoming-view ongoing-view">
            <div className="program-heading">
              ...
              <div className="udbc-programme-copy" key={activeOngoing}>
                <p className="program-pathway">{ongoingPrograms[activeOngoing].pathway}</p>
                ...
                <div className="programme-pathway-controls" role="tablist" aria-label="Lisha Kondoo Zangu study pathways">
                  {ongoingPrograms.map((program, index) => <button key={program.level} type="button" role="tab" aria-selected={activeOngoing === index} aria-label={`Show ${program.level} ${program.pathway} pathway`} onClick={() => setActiveOngoing(index)}><span /></button>)}
                </div>
                <a className="button" href={`${UDBC_URL}/intake-landing`}>Enrol Now</a>
              </div>
              <div className="program-preview">
                <img src={ongoingPrograms[activeOngoing].image} alt={ongoingPrograms[activeOngoing].imageAlt} loading="lazy" decoding="async" />
                <div className="program-preview-label"><strong>Lisha Kondoo Zangu</strong><span>{ongoingPrograms[activeOngoing].pathway}</span></div>
              </div>
            </div>
          </div>
        ) : (
          <div role="region" aria-label="Upcoming programs" className="upcoming-view">
            ...
            <div className="program-layout">
              <div>
                ...
                <a className="button" href={UDBC_URL}>Book a Slot</a>
                <button className="next-program" onClick={() => setActiveProgram((activeProgram + 1) % programs.length)}>Next <ArrowRight /></button>
              </div>
              <div className="program-preview program-image-preview"><img src={upcomingProgramArtwork} alt="Teachers Change the world classroom programme" loading="lazy" decoding="async" /></div>
            </div>
          </div>
        )}
    ...
    <section id="schools" className="listings"><div className="container"><header><h2>Explore Our School Listings</h2><p>Explore verified schools available in Sqooli. Access programmes, lessons and tutors seamlessly.</p></header><form className="listing-search" onSubmit={event => { event.preventDefault(); setListingNotice(`${visibleListings.length} verified school listing${visibleListings.length === 1 ? '' : 's'} found`) }}><Search aria-hidden="true" /><label htmlFor="school-search">Search school listings</label><input id="school-search" type="search" aria-controls="listing-results" value={listingQuery} onChange={event => setListingQuery(event.target.value)} placeholder="Type school name..." /><button type="submit">Search</button></form><p className="sr-only" aria-live="polite">{visibleListings.length} verified listings found</p><div id="listing-results" className="listing-rows">{visibleListings.map((listing, index) => <article className="listing-row" key={listing.name}><div className={`listing-thumb thumb-${index}`} aria-hidden="true">U</div><div className="listing-main"><h3>{listing.name}</h3><span>{listing.website.replace('https://', '')}</span><p><span><Users aria-hidden="true" />{listing.students}</span><span><MapPin aria-hidden="true" />{listing.location}</span></p><strong>Ufufuo Digital Bible College</strong></div><div className="listing-actions"><a href={listing.website} target="_blank" rel="noreferrer" className="button button-small" style={{ background: 'transparent', border: '1px solid #168dcc', color: '#168dcc' }}>Go to Website</a><a href="/schools/detail" className="button button-small" style={{ background: 'transparent', border: '1px solid #168dcc', color: '#168dcc' }}>View Lessons</a><a href="/schools/tutors" className="button button-small" style={{ background: 'transparent', border: '1px solid #168dcc', color: '#168dcc' }}>View Tutors</a></div></article>)}</div>{visibleListings.length === 0 && <p className="no-listings" role="status">No verified school listings found. Try another name.</p>}{listingNotice && <p className="listing-notice" role="status">{listingNotice}</p>}<a className="listing-cta" href="/schools"><ExternalLink aria-hidden="true" />Go to School Listing</a></div></section>
          ...
          {QA_CARDS.map((card, index) => (
            ...
            >
              ...
              <div className="qa-card-author-row">
                <img src={lucyAvatar} alt={card.author} className="qa-author-img" loading="lazy" decoding="async" />
    ...
    <section id="join" className="join-flow"><div className="container"><img src={childrenCollage} alt="Children learning together" loading="lazy" decoding="async"/><div><h2>Join World's largest learning platform today</h2><p>Join thousands of learners who are upgrading their skills across tutors, classes, schools and more.</p><a className="button" href="/onboarding">Get Started</a></div></div></section>
    <section id="contact" className="contact-flow">
      <div className="container contact-container">
        ...
        <div className="contact-art-side" aria-hidden="true">
          ...
          <img className="contact-kids-img" src={contactKids} alt="Children learning together" loading="lazy" decoding="async" />
        </div>
      </div>
    </section>
