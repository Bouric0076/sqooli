...
export default function TeacherCreateLessonModal({ onClose, onSubmitted }: Props) {
	...
	const addSection = () => setSections((items) => [...items, { title: `Section ${items.length + 1}`, lectures: [] }])
	const addLecture = (sectionIndex: number) => setSections((items) => items.map((section, index) => index === sectionIndex ? { ...section, lectures: [...section.lectures, { title: `Lecture ${section.lectures.length + 1}`, description: '', items: [] }] } : section))
	const updateLecture = (sectionIndex: number, lectureIndex: number, update: Partial<Lecture>) => setSections((items) => items.map((section, index) => index === sectionIndex ? { ...section, lectures: section.lectures.map((lecture, itemIndex) => itemIndex === lectureIndex ? { ...lecture, ...update } : lecture) } : section))
	const removeLecture = (sectionIndex: number, lectureIndex: number) => setSections((items) => items.map((section, index) => index === sectionIndex ? { ...section, lectures: section.lectures.filter((_, itemIndex) => itemIndex !== lectureIndex) } : section))
	const addContent = (sectionIndex: number, lectureIndex: number, item: string) => setSections((items) => items.map((section, index) => index === sectionIndex ? { ...section, lectures: section.lectures.map((lecture, itemIndex) => itemIndex === lectureIndex ? { ...lecture, items: [...lecture.items, item] } : lecture) } : section))

	return <div className="teacher-create-lesson-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
		<section className="teacher-create-lesson" role="dialog" aria-modal="true" aria-labelledby="create-lesson-title">
			<header className="teacher-create-lesson__top"><span><BookOpen size={15} /> Lessons <ChevronRight size={14} /> <b id="create-lesson-title">Create Lesson</b></span><button type="button" aria-label="Close create lesson" onClick={onClose}><X size={22} /></button></header>
...
function RichText({ title, subtitle, value, setValue }: { title: string; subtitle: string; value: string; setValue: (value: string) => void }) { return <StepPanel title={title} subtitle={subtitle}><RichEditor label={title} value={value} onChange={setValue} /></StepPanel> }
function RichEditor({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="teacher-create-lesson__label">{label}<textarea className="is-large" value={value} onChange={(event) => onChange(event.target.value)} placeholder="Enter a description..." /><span className="teacher-create-lesson__toolbar">Undo / Redo · Sans Serif · <b>B</b> · <i>I</i> · <u>U</u> · Align · Lists</span></label> }

type LessonContentProps = { sections: Section[]; updateSection: (index: number, update: Partial<Section>) => void; addSection: () => void; addLecture: (index: number) => void; updateLecture: (sectionIndex: number, lectureIndex: number, update: Partial<Lecture>) => void; removeLecture: (sectionIndex: number, lectureIndex: number) => void; addContent: (sectionIndex: number, lectureIndex: number, item: string) => void }
function LessonContent({ sections, updateSection, addSection, addLecture, updateLecture, removeLecture, addContent }: LessonContentProps) { return <StepPanel title="Lesson Content" subtitle="Select the type of content that will be included in your lesson">{sections.map((section, sectionIndex) => <div className="teacher-create-lesson__section" key={sectionIndex}><div className="teacher-create-lesson__section-head"><input value={section.title} onChange={(event) => updateSection(sectionIndex, { title: event.target.value })} /><button type="button" aria-label="Delete section" onClick={() => sections.length > 1 && updateSection(sectionIndex, { lectures: [] })}><Trash2 size={14} /></button></div>{section.lectures.map((lecture, lectureIndex) => <div className="teacher-create-lesson__lesson" key={lectureIndex}><div className="teacher-create-lesson__lesson-head"><input value={lecture.title} onChange={(event) => updateLecture(sectionIndex, lectureIndex, { title: event.target.value })} /><button type="button" aria-label="Edit lecture"><Edit3 size={14} /></button><button type="button" aria-label="Delete lecture" onClick={() => removeLecture(sectionIndex, lectureIndex)}><Trash2 size={14} /></button></div><label>Description<textarea value={lecture.description} onChange={(event) => updateLecture(sectionIndex, lectureIndex, { description: event.target.value })} placeholder="Enter description..." /></label>{lecture.items.length > 0 && <div className="teacher-create-lesson__content-items">{lecture.items.map((item, itemIndex) => <span key={itemIndex}>{item}<button type="button" onClick={() => updateLecture(sectionIndex, lectureIndex, { items: lecture.items.filter((_, index) => index !== itemIndex) })}><X size={12} /></button></span>)}</div>}<div className="teacher-create-lesson__chips">{['Quiz', 'Assignment', 'Exam', 'Other Resources', 'Video'].map((item) => <button type="button" key={item} onClick={() => addContent(sectionIndex, lectureIndex, item)}><Plus size={14} /> {item}</button>)}</div></div>)}<button type="button" className="teacher-create-lesson__outline-button" onClick={() => addLecture(sectionIndex)}><Plus size={15} /> Add Lecture</button></div>)}<button type="button" className="teacher-create-lesson__section-add" onClick={addSection}><Plus size={15} /> Add Section</button></StepPanel> }

function Pricing({ price, setPrice, revenueRows, onRevenue, onRemoveRevenue }: { price: string; setPrice: (value: string) => void; revenueRows: string[]; onRevenue: () => void; onRemoveRevenue: (index: number) => void }) { const amount = Number(price) || 0; const fee = amount * .2; return <StepPanel title="Pricing" subtitle="Setup your pricing for each lesson"><div className="teacher-create-lesson__pricing"><label>Rate per Lesson<small>Total client will see</small><input inputMode="decimal" value={`KES ${price}`} onChange={(event) => setPrice(event.target.value.replace(/\D/g, ''))} /></label><label>Sqooli Service Fee<small>Sqooli charges 20% on every transaction</small><input value={`KES ${fee.toFixed(2)}`} readOnly /></label><label>What you will receive<small>The estimated amount after service Payments</small><input value={`KES ${(amount - fee).toFixed(2)}`} readOnly /></label></div><h3>Do you have additional revenue shares?</h3><p>Add other custom deductions on your earnings</p><button type="button" className="teacher-create-lesson__outline-button" onClick={onRevenue}><Plus size={15} /> Add Revenue Shares</button>{revenueRows.length > 0 && <div className="teacher-create-lesson__revenue-table"><b>User</b><b>Commission Type</b><b>Value</b><b>Actions</b>{revenueRows.map((row, index) => <div key={`${row}-${index}`}><span>{row}</span><span>Fixed Value</span><span>50</span><span><button type="button"><Edit3 size={14} /></button><button type="button" onClick={() => onRemoveRevenue(index)}><Trash2 size={14} /></button></span></div>)}</div>}</StepPanel> }
